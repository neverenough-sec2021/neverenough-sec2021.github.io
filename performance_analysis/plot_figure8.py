...
def load_data(args, key):
    ...
    tgen_stats_paths = get_filepaths(args.inpath, key)
    ...
    for path in tgen_stats_paths:
        ...
        for errtype in data.keys():
            db[load][net].setdefault(errtype, [])
            db[load][net][errtype].extend(data[errtype])
...
def run(args):
    ...
    for scale in [1,10,30]:
        ...
        err_db[scale] = load_data(args, "error_rate")

    logging.info("Starting to plot")
    set_plot_options()
    matplotlib.rcParams['ytick.major.pad']='0.0'

    stats_filename = "tgen-err-stats.txt"
    if os.path.exists(stats_filename):
        os.remove(stats_filename)

    plot(err_db, "ALL", "Error Rate (\%)", "err", 1, [10, 100], "figure8a.pdf")
    plot(err_db, "ALL", "Error Rate (\%)", "err", 10, [5, 10, 100], "figure8b.pdf")
    plot(err_db, "ALL", "Error Rate (\%)", "err", 30, [5, 10], "figure8c.pdf")
